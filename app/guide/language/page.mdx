...
- `components/layout/language-switcher.tsx` — UI component for switching locales  
- `i18n/routing.ts` — defines supported locales and localized paths  
- `i18n/messages/*` — locale message modules (`en`, `ur`, etc.)  
- `app/[locale]/layout.tsx` — root layout providing messages via `NextIntlClientProvider`  
- `middleware.ts` — uses `next-intl/middleware` to enforce locale-prefixed routes  
...
This navigates to the same path with the selected locale (e.g., `/en/dashboard` → `/ur/dashboard`).

<Callout type="note">
ℹ️ Routes are locale-prefixed (e.g., `/en/login` or `/ur/login`). The routing file defines all available locales and their path mapping.
</Callout>
...
5. Restart the dev server — the `LanguageSwitcher` will show the new locale automatically.

<Callout type="tip">
💡 Update the `locales` array inside `language-switcher.tsx` if you show flags or names in the UI.
</Callout>
...
⚠️ Removing i18n is destructive — keep a backup branch before performing these steps.
</Callout>
...
<Callout type="tip">
💡 Keep message keys identical across locales. Missing keys fallback to the default locale.
</Callout>
